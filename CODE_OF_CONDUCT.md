# Code of Conduct

## Our Commitment

Needle is a community resource built on harm reduction principles. We
are committed to providing a welcoming, respectful, and supportive environment for all contributors,
regardless of their background, identity, or experience with drug use.

---

## Harm Reduction Values

This project is grounded in core harm reduction principles:

### 1. Respect for People Who Use Drugs

- People who use drugs are the experts on their own lives
- Lived experience is valued knowledge
- We use non-stigmatizing, person-first language
- We reject moral judgments about drug use

### 2. Meeting People Where They Are

- We support people at all stages of their journey
- We don't require abstinence as a condition of participation
- We recognize that harm reduction works on a spectrum

### 3. Nothing About Us Without Us

- People with lived experience lead and inform this work
- Peer voices are centered, not tokenized
- We actively seek input from affected communities

### 4. Evidence-Based & Pragmatic

- We prioritize what works, not ideology
- We value public health over criminalization
- We support policy based on evidence, not stigma

---

## Community Standards

### Expected Behavior

- **Be respectful** - Treat all community members with dignity
- **Use inclusive language** - Avoid stigmatizing terminology
- **Welcome newcomers** - Support people learning about harm reduction
- **Give constructive feedback** - Focus on improving the work
- **Center lived experience** - Elevate voices of people who use drugs
- **Practice humility** - Be open to learning and correction
- **Protect privacy** - Never share identifying information about individuals

### Unacceptable Behavior

- Stigmatizing language about people who use drugs
- Harassment, discrimination, or personal attacks
- Sharing private or identifying information
- Content that could facilitate harm to individuals
- Dismissing or devaluing lived experience
- Promoting punitive or abstinence-only approaches
- Any form of bullying or intimidation

---

## Language Matters

We use language that respects the dignity of people who use drugs:

| Preferred                          | Avoid                |
| :--------------------------------- | :------------------- |
| In recovery, not currently using   | Clean                |
| Medication for opioid use disorder | Replacement therapy  |
| Overdose, poisoning                | OD'd (casually)      |
| Person who uses drugs              | Addict, junkie, user |
| Positive/negative (test results)   | Clean/dirty          |
| Substance use                      | Substance abuse      |

> [!NOTE] When researching historical documents, you may need to use older terminology in search
> queries. This is acceptable for research purposes—context matters.

---

## Scope

This Code of Conduct applies to:

- All contributions to this repository
- Issues, pull requests, and discussions
- Any community spaces associated with this project
- Representing the project in public spaces

---

## Reporting Issues

If you experience or witness behavior that violates this Code of Conduct:

1. **Document the incident** - Note what happened, when, and who was involved
2. **Report it** - Contact project maintainers via [issues](../../issues) or direct message
3. **Seek support** - You don't have to handle this alone

### What Happens Next

- Reports will be reviewed promptly and confidentially
- We will take appropriate action, which may include:
  - A warning to the person involved
  - Temporary or permanent removal from the project
  - Other actions as appropriate

---

## Enforcement

Project maintainers are responsible for clarifying and enforcing standards of acceptable behavior.
They have the right and responsibility to:

- Remove, edit, or reject contributions that violate this Code of Conduct
- Temporarily or permanently ban contributors for inappropriate behavior
- Make decisions in the best interest of the community

---

## Attribution

This Code of Conduct is adapted from the
[Contributor Covenant](https://www.contributor-covenant.org/) and informed by harm reduction
principles from organizations including:

- [Harm Reduction International](https://hri.global/)
- [National Harm Reduction Coalition](https://harmreduction.org/)
- [Australian Injecting & Illicit Drug Users League (AIVL)](https://aivl.org.au/)

---

## Questions?

If you have questions about this Code of Conduct or need clarification on expected behavior, please
open an issue or contact project maintainers.

---

_This Code of Conduct reflects our commitment to harm reduction values and creating a supportive
community for everyone working to reduce drug-related harm._
