...
parser.add_argument(
    "-n", "--number_of_pages", type=int, required=True, help="Number of pages to search"
)
parser.add_argument(
    "-t", "--time", type=int, required=True, help="Time between requests"
)
...
try:
    ...
    for dork_line in dorks:
            ...
            if search_in_vdp == "yes" or search_in_vdp == "y":
                ...
                if grep_process.returncode == 0:
                    print(
                        ...
                        + "Note:"
                        + "\033[0m \033[0;32m"
                        + f"This Website ( {extracted_domain} ) is vulnerable and has VDP/bbp so, you can report them legally!"
                        + "\033[0m"
                    )
            counter += 1
        ...
        if urls:
            saved_results.append({"dork_line": dork_line, "urls": urls})
