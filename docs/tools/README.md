# Tools & Automation

> Browser tools, automation, and resources to enhance your searching.

[← Back to Main Guide](../../README.md)

---

## 📋 Available Tools Guides

| Guide                                       | Description                    |
| ------------------------------------------- | ------------------------------ |
| [AI Tools](ai-tools.md)                     | AI-powered research assistants |
| [Bookmarklets](bookmarklets.md)             | One-click browser searches     |
| [Browser Extensions](browser-extensions.md) | Research workflow tools        |
| [Custom Search](custom-search.md)           | Build team search engines      |
| [Google Alerts](google-alerts.md)           | Automated monitoring           |
| [OSINT](osint.md)                           | Open source intelligence tools |
| [Privacy & Security](privacy-security.md)   | Protect yourself & clients     |

---

## 🚀 Quick Start

### Essential Tools for Getting Started

1. **[Unpaywall](https://unpaywall.org/)** - Find free versions of academic papers
2. **[Wayback Machine](https://web.archive.org/)** - Recover broken links
3. **[Google Alerts](google-alerts.md)** - Automated monitoring

### For Advanced Users

1. **[OSINT Tools](osint.md)** - Deep investigation
2. **[Privacy Tools](privacy-security.md)** - Secure searching
3. **[AI Research](ai-tools.md)** - AI-powered analysis

---

## 💡 Tips

- Start with bookmarklets for common searches
- Set up Google Alerts for your key topics
- Install Unpaywall to access research papers
- Use privacy tools for sensitive searches

---

[← Back to Main Guide](../../README.md)
