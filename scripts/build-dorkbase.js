...
const processFiles = () => {
    ...
    const files = fs.readdirSync(DORK_PACKS_DIR).filter(file => file.endsWith('.md') && file !== 'README.md');
    const dorkDatabase = [];
    ...
    files.forEach(file => {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.startsWith('## ') && !line.startsWith('### ')) {
                ...
                if (currentDork) {
                    if (explanationBuffer.length > 0) currentDork.explanation = explanationBuffer.join('\n').trim();
                    currentPack.dorks.push(currentDork);
                    currentDork = null;
                    captureExplanation = false;
                    explanationBuffer = [];
                }
                currentCategory = line.replace('## ', '').replace(/🟢|🟡|🔴|⚡|📊|🔬|🏛️|🎓|💊|📍|🔗|🌏|alert/g, '').trim(); // Strip emojis
                continue;
            }
    ...
    const cleanDatabase = dorkDatabase.map(pack => ({
        ...pack,
        dorks: pack.dorks.filter(d => d.query && d.query.length > 0 && !d.query.includes('| State | Query |')) 
    }));
